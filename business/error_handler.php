...
class ErrorHandler 
{
	// Private ctor to prevent direct creation of object
	...
	{
		// The first two elements of backtrace aray are irrelevant
		$backtrace = ErrorHandler::GetBacktrace(2);

		$error_message = "\nERRNO: $errNo\nTEXT: $errStr, "
			. "line $errLine, at " . date('F j, Y, g:i 1') .
			"\nShowing backtrace:\n$backtrace\n\n";

		// Email the error details, in case SEND_ERROR_MAIL is true
		if (SEND_ERROR_MAIL == true) 
		{
			error_log($error_message, 1, ADMIN_ERROR_MAIL,
				"From: " . SENDMAIL_FROM . "\r\nTo: " 
				. ADMIN_ERROR_MAIL);
		}
